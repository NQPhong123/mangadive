...
class _EditMangaScreenState extends State<EditMangaScreen> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      String coverImageUrl = widget.manga.coverImage;

      // Nếu có ảnh mới, upload lên Cloudinary
      if (_coverImage != null) {
        coverImageUrl = await _mangaService.uploadImage(_coverImage!);
      }
      ...
      final updatedManga = Manga(
        ...
        viewCount: widget.manga.viewCount,
        createdAt: widget.manga.createdAt,
        updatedAt: DateTime.now(),
        chapters: widget.manga.chapters,
      );

      await _mangaService.updateManga(updatedManga);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Cập nhật manga thành công')),
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Chỉnh Sửa Manga')),
